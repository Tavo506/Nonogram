...
public class Logic : MonoBehaviour
{
    ...
    
    {
        ...
        for (int i = 0; i < x; i++) {
            ...
            Debug.Log(i);
            Debug.Log(filas[i].ToString().Split(',').Length);
            RellenaParcialFilas(i, 0, 0, 0); 
        }
    ...
    bool probarF(int i, int j)
    {
        ...
        if (filas[i].ToString().Replace(",", "").Replace(" ", "") == recorredor.Replace(",", "") && probarCX(j))
        {
    ...
    bool probarCX(int j)
    {
        ...
        string[] a = columnas[j].ToString().Replace(" ", "").Split(',');
        int n = 0;

        for (int i = 0; i < a.Length; i++)
            n += int.Parse(a[i]);
        if (cuantoshaycolumna(j) > n)
            return false;
        else
            return true;
    }
    ...
    bool probarFX(int j)
    {
        ...
        string[] a = filas[j].ToString().Replace(" ", "").Split(',');
        int n = 0;

        for (int i = 0; i < a.Length; i++)
            n += int.Parse(a[i]);

        if (cuantoshayfila(j) > n)
            return false;
        else
            return true;
    }
    ...
    bool probarC(int i, int j)
    {
        ...
        if (i == 7 && j == 9)
            Debug.Log(recorredor + " - " + columnas[j]);
    ...
    {
        string[] pista = filas[i].ToString().Split(',');
        int len = pista.Length;
        if (matriz[i, j] != 0)
        {
            if (matriz[i, j] == -1)
                matriz[i, j] = 2;
